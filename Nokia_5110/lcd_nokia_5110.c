...
static void send_data(uint8_t data) {
   ...
   digitalWrite(LCD_PIN_SCLK, LOW); /* make sure is LOW */
   digitalWrite(LCD_PIN_DC, HIGH); /* set to HIGH for send data */
...
static void send_command(uint8_t command) {
   ...
   digitalWrite(LCD_PIN_SCE, HIGH);
   digitalWrite(LCD_PIN_SCLK, LOW); /* make sure is LOW */
   digitalWrite(LCD_PIN_DC, LOW); /* set to LOW for send commands */
   /* NOTE(elsuizo:2018-03-30): luego de esto necesitamos enviar una trama de bits como en la figura, para ello podemos
      utilizar la funcion de Arduino shiftOut(dataPin, clockPin, bitOrder, value)
    */
   digitalWrite(LCD_PIN_SCE, LOW);
   shiftOut(LCD_PIN_SDIN, LCD_PIN_SCLK, MSBFIRST, command);
   /* after send command make sure SCE is HIGH!!! */
   digitalWrite(LCD_PIN_SCE, HIGH);
}
